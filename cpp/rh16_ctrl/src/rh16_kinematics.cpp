...
class PinocchioRh16 : public rclcpp::Node
{
  
public:
  PinocchioRh16( std::string name ) : Node( name )
  {

    RCLCPP_INFO(this->get_logger(), "hello z13 %s.",name.c_str());

    // 加载 URDF 模型
    std::string urdf_path = ament_index_cpp::get_package_share_directory("rh16_ctrl") + "/urdf/rh1621hand_z.urdf";
    ...
    ik_subscriber_ = this->create_subscription<geometry_msgs::msg::Pose>(
      "ik_target", 10, std::bind(&PinocchioRh16::ik_callback, this, std::placeholders::_1));

    // 定时执行正向运动学
    timer_ = this->create_wall_timer(50ms, std::bind(&PinocchioRh16::fk_callback, this));
...
private:
  ...
  void fk_callback()
  {
    ...
    // 获取末 关节坐标系的位姿
    // RCLCPP_INFO(this->get_logger(), "fz16 %d", model_.getJointId("fz16") );
    pinocchio::SE3 end_effector_pose1 = data_.oMi[ model_.getJointId("fz16") ];
  ...
  void ik_callback(const geometry_msgs::msg::Pose::SharedPtr msg)
  {
    ...
    for (int i = 0; i < IT_MAX; ++i)
    {
      ...
      const pinocchio::SE3 iMd = data_.oMi[model_.getJointId("fz16")].actInv(target_pose);
      ...
      pinocchio::computeJointJacobian(model_, data_, q_ik, model_.getJointId("fz16"), J); // 计算雅可比矩阵
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<PinocchioRh16>("rh16_pinocchio");
