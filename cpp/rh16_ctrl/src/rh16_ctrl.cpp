...
rclcpp::Node::SharedPtr g_node = nullptr;  // 用于信号处理的节点指针
...
namespace ruiyan::rh16
{
    ...
    void rh16_ctrl::UpdataMotor( void ) //电机参数下发
    {
	    ...
	    RyMotion_ServoMove_Mix(&stuServoCan, 16, LOCK_POS_16, 1000, 100, &sutServoDataR[15], 1);    
    }
        ...
        if (memcmp(&rh16cmd, &cmd, sizeof(rh16_cmd::msg::Rh16Cmd)))
        {
            ...
            switch (cmd.mode)
            {
                ...
                case 2:
                {
                    ...
                    for (int i = 0; i < 21; i++)
                    {
                        if (i % 4 == 3)
                        {
                            ...
                            // 直接映射每个关节到对应的电机
                            sutServoDataW[(i / 4 * 3) + 0].stuCmd.usTp = p1;
                            sutServoDataW[(i / 4 * 3) + 1].stuCmd.usTp = p2;
                ...
                case 1:
                {
                    // 重新设计：21个关节到电机的映射
                    // 拇指：5个关节，但只需要映射到4个电机
                    p1 = map_rad_to_value_full_range(cmd.j_ang[0]);  // 拇指关节1 -> 电机0
                    p2 = map_rad90_to_value(cmd.j_ang[1]);           // 拇指关节2 -> 电机1
                    p3 = map_rad75_to_value(cmd.j_ang[2]);            // 拇指关节3 -> 电机2
                    p4 = map_rad75_to_value(cmd.j_ang[3]);            // 拇指关节4 -> 电机3
                    ...
                    sutServoDataW[0].stuCmd.usTp = p1;  // 拇指关节1
                    sutServoDataW[1].stuCmd.usTp = p2;  // 拇指关节2
                    sutServoDataW[2].stuCmd.usTp = p3;  // 拇指关节3
                    sutServoDataW[3].stuCmd.usTp = p4;  // 拇指关节4

                    // 处理其他4个手指，每个4个关节
                    for (int finger = 1; finger < 5; finger++)
                    {
                        int base_j_idx = 5 + (finger - 1) * 4;  // j_ang数组中的起始索引
                        int base_m_idx = finger * 3;            // 电机数组中的起始索引

                        p1 = map_rad_to_value_full_range(cmd.j_ang[base_j_idx + 0]);
                        p2 = map_rad90_to_value(cmd.j_ang[base_j_idx + 1]);
                        p3 = map_rad75_to_value(cmd.j_ang[base_j_idx + 2]);
                        p4 = map_rad75_to_value(cmd.j_ang[base_j_idx + 3]);
                        (void)p4;
                        ...
                        sutServoDataW[base_m_idx + 0].stuCmd.usTp = p5;
                        sutServoDataW[base_m_idx + 1].stuCmd.usTp = p6;
                        sutServoDataW[base_m_idx + 2].stuCmd.usTp = p3;
                        sutServoDataW[base_m_idx + 3].stuCmd.usTp = p4;
                    }
                    UpdataMotor();
                }
                break;
    ...
    void rh16_ctrl::PubState()
    {
        ...
        int p, v, t, pcom, perr;
         ...
         for(int i = 0; i < 16; i++) {
            RyFunc_GetServoInfo( &stuServoCan, i+1, &sutServoDataR[i], 0 );
        }
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            // 拇指：5个关节，使用前4个电机 (i = 0,1,2,3)
            if (i < 4) {
                switch(i) {
                    case 0: // 拇指关节1
                        rh16msg.j_ang[0] = value_to_rad_full_range(p);
                        break;
                    case 1: // 拇指关节2
                        rh16msg.j_ang[1] = value_to_rad90(p);
                        break;
                    case 2: // 拇指关节3
                        rh16msg.j_ang[2] = value_to_rad75(p);
                        break;
                    case 3: // 拇指关节4+5（需要特殊处理，第5个关节基于第4个电机位置计算）
                        rh16msg.j_ang[3] = value_to_rad75(p);
                        rh16msg.j_ang[4] = deg_to_rad(evaluatePolynomial(poly_coeff[0], 5, rad_to_deg(value_to_rad75(p))));
                        break;
                }
            }
            // 其他手指：每个4个关节，使用12个电机 (i = 4-15)
            else {
                int finger_index = (i - 4) / 3;  // 0-3 对应食指、中指、无名指、小指
                int joint_in_finger = (i - 4) % 3;  // 0-2 对应每个手指的3个关节
                int j_ang_base = 5 + finger_index * 4;  // 拇指5关节后，每个手指4关节

                if(joint_in_finger < 3) {
                    switch(joint_in_finger) {
                        case 0: // 每个手指的第1个关节
                            rh16msg.j_ang[j_ang_base + 0] = value_to_rad_full_range(p);
                            break;
                        case 1: // 每个手指的第2个关节
                            rh16msg.j_ang[j_ang_base + 1] = value_to_rad90(p);
                            break;
                        case 2: // 每个手指的第3个关节，并计算第4个关节
                            rh16msg.j_ang[j_ang_base + 2] = value_to_rad75(p);
                            rh16msg.j_ang[j_ang_base + 3] = deg_to_rad(evaluatePolynomial(poly_coeff[finger_index + 1], 5, rad_to_deg(value_to_rad75(p))));
                            break;
                    }
                }
            }
        ...
        // 将21个关节角度转换为 Pinocchio 所需的格式
        // URDF中的关节顺序：拇指6个(fz1611-fz1616)，其他手指各5个
        // 但实际j_ang是：拇指5个(0-4)，其他手指各4个(5-20)

        // 拇指：5个关节映射到URDF的前5个关节
        q_[0] = rh16msg.j_ang[0];  // fz1611
        q_[1] = rh16msg.j_ang[1];  // fz1612
        q_[2] = rh16msg.j_ang[2];  // fz1613
        q_[3] = rh16msg.j_ang[3];  // fz1614
        q_[4] = rh16msg.j_ang[4];  // fz1615

        // 其他手指：各4个关节
        // 食指
        q_[6] = rh16msg.j_ang[5];   // fz1621
        q_[7] = rh16msg.j_ang[6];   // fz1622
        q_[8] = rh16msg.j_ang[7];   // fz1623
        q_[9] = rh16msg.j_ang[8];   // fz1624

        // 中指
        q_[11] = rh16msg.j_ang[9];  // fz1631
        q_[12] = rh16msg.j_ang[10]; // fz1632
        q_[13] = rh16msg.j_ang[11]; // fz1633
        q_[14] = rh16msg.j_ang[12]; // fz1634

        // 无名指
        q_[16] = rh16msg.j_ang[13]; // fz1641
        q_[17] = rh16msg.j_ang[14]; // fz1642
        q_[18] = rh16msg.j_ang[15]; // fz1643
        q_[19] = rh16msg.j_ang[16]; // fz1644

        // 小指
        q_[21] = rh16msg.j_ang[17]; // fz1651
        q_[22] = rh16msg.j_ang[18]; // fz1652
        q_[23] = rh16msg.j_ang[19]; // fz1653
        q_[24] = rh16msg.j_ang[20]; // fz1654

        // 设置耦合关节（根据URDF的定义）
        q_[5] = q_[4];   // fz1616 = fz1615 (拇指)
        q_[10] = 0;      // fz1625 = 0 (固定关节)
        q_[15] = 0;      // fz1635 = 0 (固定关节)
        q_[20] = 0;      // fz1645 = 0 (固定关节)
        q_[25] = 0;      // fz1655 = 0 (固定关节)
...
int main(int argc, char *argv[])
{
    ...
    // 释放申请的内存，避免内存泄漏
    if(stuServoCan.pstuHook != NULL) {
        free(stuServoCan.pstuHook);
        stuServoCan.pstuHook = NULL;
    }

    if(stuServoCan.pstuListen != NULL) {
        free(stuServoCan.pstuListen);
        stuServoCan.pstuListen = NULL;
    }
